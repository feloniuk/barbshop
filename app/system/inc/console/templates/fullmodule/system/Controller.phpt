    ...
    public function addAction()
    {
        if ($this->startValidation()) {
            ...
            if ($this->isValid()) {
                $data = [
                    'title' => post('title'),
                    'slug'  => Model::createIdentifier('{{ tableName }}', makeSlug(post('title'))),
                    'time'  => time()
                ];
    ...
    public function editAction()
    {
        ...
        if ($this->startValidation()) {
            ...
            $this->validatePost('image',        'Image',                'required|trim|min_length[1]|max_length[100]');
             $this->validatePost('file',        'File',                 'required|min_length[1]|max_length[100]|is_file');
            ...
            if ($this->isValid()) {
                $data = array(
                    ...
                    'posted'  => post('posted'),
                    'slug'    => Model::createIdentifier('{{ tableName }}', post('slug'), 'slug', $this->view->edit->id),
                    'time'    => time()
                );

                // Copy and remove image
                if ($this->view->edit->image !== $data['image']) {
                    Storage::entityImage('{{ classNamePath }}', $id, $data['image'], $this->view->edit->image,
                        600, 400, true);
                }

                //Copy file
                if ($data['file'] && $this->view->edit->file !== $data['file']) {
                    if (Storage::copy('data/tmp/' . $data['file'], 'data/{{ classNamePath }}/' . $data['file'])) {
                        Storage::remove('data/{{ classNamePath }}/' . $this->view->edit->file);
                    } else
                ...
                if ($result) {
                   //save alt attributes
                    ImageAlts::saveAlts(post('alt_attributes'), '{{ tableName }}', $id);

                    //Versions
                    $this->makeVersion($id, '{{ tableName }}', 'edit');

                     // Remove and after insert sectors
                    {{ className }}Model::removeSectors($id);
                    if (post('sector_ids')) {
                        foreach (post('sector_ids') as $sector_id) {
                            Model::insert('{{ tableName }}_sectors', array(
                                '{{ singTableName }}_id' => $id,
                                'sector_id' => $sector_id
                            ));
                        }
                    }
