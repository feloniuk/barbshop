...
class {{ className }}Model extends Model
{
    ...
    public function module_install()
    {
        $queries = [
            "CREATE TABLE IF NOT EXISTS `{{ tableName }}` (
             ...
             "CREATE TABLE IF NOT EXISTS `{{ tableName }}_sectors` (
                `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
                `{{ singTableName }}_id` int(10) unsigned NOT NULL,
    ...
    public static function get($id)
    {
        ...
        $item = self::fetch(self::query($sql));
         ...
         if ($item) {
            ...
            $item = self::relationship($item, '{{ tableName }}', 'sectors');
        }
    ...
    public static function removeSectors($vid)
    {
        $sql = "
            DELETE
            FROM `{{ tableName }}_sectors`
            WHERE `{{ singTableName }}_id` = '$vid'
        ";
